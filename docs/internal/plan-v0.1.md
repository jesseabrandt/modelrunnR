...
**Status**: **Historical — shipped.** All 11 slices landed as of the audit
series (commits `b65abca` through `123955a`). Retained as the vocabulary and
sequencing reference for v0.1; the current design in `docs/design.md` has
since been extended by the swappability merge and the `launch(pin=, data=)`
and pre-swappability `grab()` signatures referenced below are superseded.
Follow-up work for swappability lives in a separate plan.

**Last updated**: 2026-04-08 (archived 2026-04-09)
